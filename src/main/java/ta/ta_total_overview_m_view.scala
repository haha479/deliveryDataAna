...
/**
  ...
  * @author 479
  */
object ta_total_overview_m_view {
  def main(args: Array[String]): Unit = {
    ...
    import spark.sql
    sql("DROP VIEW IF EXISTS waimai.ta_total_overview_m_view")
    sql(
      """
        |CREATE VIEW IF NOT EXISTS waimai.ta_total_overview_m_view
        |AS
        |SELECT
        |b.month_id ,
        |a.province_id,
    ...
    spark.sql("select * from waimai.ta_total_overview_m_view").show(false)
